    ...
    <div style="display: flex">
      ...
      <div class="all-box-left">
        <el-card class="box">
          ...
          <div class="question" v-for="(item, index) in tableData" :key="index">
            <el-tooltip :content="item.content" placement="top">
              <div class="qu-content">{{ item.content }}</div>
            </el-tooltip>
            <el-tag>{{ item.quType }}</el-tag>
            <el-tag>{{ item.questionBankName }}</el-tag>
            <el-tag>级别:{{ item.level }}</el-tag>
          </div>
      ...
      <div class="all-box-center">
        ...
        <div>
          <h3>题目列表</h3>
          <template v-if="!questionBankList">
            <div>你好</div>
          </template>
