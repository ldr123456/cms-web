                            ...
                            <el-dropdown-menu class="dropdown-group">
                              <el-dropdown-item>
                                <span style="font-weight: 600">
                                  当前身份：{{
                                    userStore.userInfo.department.name+userStore.userInfo.authority.authorityName
                                  }}
                                </span>
                              </el-dropdown-item>
                              <template v-if="userStore.userInfo.dept_and_authority_list">
                                <el-dropdown-item
                                  v-for="item in userStore.userInfo.dept_and_authority_list.filter(
                                    (i) =>
                                      i.SysAuthorityAuthorityId !== userStore.userInfo.authorityId ||
                                      i.SysDepartmentId != userStore.userInfo.departmentId
                                  )"
                                  :key="item.SysAuthorityAuthorityId"
                                  @click="
                                    changeUserAuth(item.SysDepartmentId, item.SysAuthorityAuthorityId)
                                  "
                                >
                                  <span> 切换为：{{ item.dept_and_authority_name }} </span>
                                </el-dropdown-item>
                              </template>
                              <!--  -->
...
// 部门信息放入角色对象中
// for (let i in userStore.userInfo.departments) {
//   Object.assign(userStore.userInfo.authorities[i], {
//     name: userStore.userInfo.departments[i].name,
//     departmentId: userStore.userInfo.departments[i].ID,
//   });
// }
...
// 切换身份权限
const changeUserAuth = async (ID, id) => {
