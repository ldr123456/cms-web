...
import dictDataDialog from "@/view/systemSet/component/dictDataDialog.vue";
import {
  createSysDataDictionary,
  deleteSysDataDictionary,
  updateSysDataDictionary,
  getSysDataDictionaryList,
...
const initTable = async () => {
  const { data } = await getSysDataDictionaryList({
    page: page.value,
...
// 显示编辑弹窗
const editDictVisible = async (type,row) => {
  ...
  await editFormRef.value.validate(async (valid, fields) => {
    if (valid) {
      if (dialogType.value === "add") {
        var res = await createSysDataDictionary(editDiaData.value);
      } else if (dialogType.value === "edit") {
        var res = await updateSysDataDictionary(editDiaData.value);
      }
      if (res.code == 0) {
        cancelDialog();
        console.log(res);
...
const delDict = () => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const res = await deleteSysDataDictionary({
